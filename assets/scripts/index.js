var Index = function () {
    ...
    return {
        ...
        initCharts: function () {
            ...
            $.getJSON("http://localhost/kasir/home/itemChart", function(json){
			
			for(a=1;a<=31;a++){
				$.each(json, function(i, item){
					if(item.tanggal == a){								
						pageviews.push([item.tanggal, item.total]);
					}else{
						pageviews.push([a, 0]);
					}
				});
                ...
                $("#site_statistics").bind("plothover", function (event, pos, item) {
                    ...
                    if (item) {
                        if (previousPoint != item.dataIndex) {
                            ...
                            showTooltip( item.pageX, item.pageY, item.series.label + " tanggal " + x + " : Rp " + y);
                        }
                    } else {
                        $("#tooltip").remove();
                        previousPoint = null;
